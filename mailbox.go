...
func (mbox *Mailbox) uidNext() uint32 {
	...
	message := &Message{}
	err := db.Order("uid desc").First(&message).Error
...
func (mbox *Mailbox) flags() []string {
	...
	*/
	...
	messages := []Message{}
	err := db.Where(Message{MailboxID: mbox.ID}).Find(&messages).Error
	if err != nil {
		// TODO handle error
...
func (mbox *Mailbox) Status(items []imap.StatusItem) (*imap.MailboxStatus, error) {
	...
	var count int64
	db.Model(&Message{}).Where(Message{MailboxID: mbox.ID}).Count(&count)
...
func (mbox *Mailbox) CopyMessages(uid bool, seqset *imap.SeqSet, destName string) error {
	...
	*/
	...
	dest := &Mailbox{}
	err := db.Where(Mailbox{Name_: destName}).Find(&dest).Error
	if err != nil {
