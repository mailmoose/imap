...
func (u *User) ListMailboxes(subscribed bool) ([]backend.Mailbox, error) {
	...
	err := db.Where(Mailbox{UserID: u.ID}).Find(&mailboxes).Error
	if err != nil {
...
func (u *User) DeleteMailbox(name string) error {
	...
	*/
	...
	mailbox := &Mailbox{}
	err := db.Where(Mailbox{UserID: u.ID}).Where(&Mailbox{Name_: name}).Find(&mailbox).Error
	if err != nil {
		err = fmt.Errorf("mailbox does not exist: %v", err)
	}

	err = db.Delete(mailbox).Error
	if err != nil {
...
func (u *User) RenameMailbox(existingName, newName string) error {
	...
	*/
	mailbox := &Mailbox{}
	err := db.Where(Mailbox{UserID: u.ID}).Where(&Mailbox{Name_: existingName}).Find(&mailbox).Error
	if err != nil {
		err = fmt.Errorf("mailbox does not exist: %v", err)
	}

	mailbox.Name_ = newName
	err = db.Save(mailbox).Error
	if err != nil {
